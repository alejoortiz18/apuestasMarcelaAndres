...
## SWApuestas MVC responsive requirements

When building the ASP.NET MVC application, treat responsive behavior as a functional requirement, not a final visual adjustment:

- Design Razor views mobile-first and support common desktop widths without duplicating the view.
- Use responsive grid and flex layouts with stable containers; avoid fixed widths that cause horizontal scrolling.
- Make navigation, filters, forms, validation messages, action buttons, dialogs, and pagination usable on narrow screens.
- Administrative tables must remain readable: use responsive columns, controlled overflow, or a deliberate compact/mobile representation. Never allow important values or actions to become inaccessible.
- Preserve touch target size, visible keyboard focus, sufficient contrast, and readable text at mobile widths.
- Keep Bootstrap or the existing MVC design system consistent; do not add arbitrary breakpoint values or competing layout frameworks.
- Check views at mobile, tablet, and desktop widths before completion, including long labels, validation errors, empty states, loading states, and permission-dependent actions.
- Keep responsive rules close to the component or page they support and avoid global CSS that unexpectedly changes unrelated MVC views.
