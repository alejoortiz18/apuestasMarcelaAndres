...
- En **Configuración** un solo **Guardar configuración** persiste horario (apertura y cierre en formato 12 h con AM/PM; no pueden ser iguales), vigencia, límites, alertas, códigos offline, los minutos de inactividad para cerrar la sesión del administrador y del PDA, el texto parametrizable de la tirilla y el mensaje al superar el tope (también se descarga al PDA para usarlo sin conexión). La sección **Números restringidos** permite agregar y eliminar números de 3 o 4 dígitos; esos números no se pueden jugar y el PDA los descarga con la configuración. La sección **Días de venta** muestra lunes a domingo; para cada día se marcan las loterías vendibles y **Guardar días de venta** persiste esa matriz. El catálogo de loterías usa casilla, paginación y el panel **Editar** / **Deshabilitar** o **Habilitar**. Al **Agregar lotería** se pide al menos un día de juego.
